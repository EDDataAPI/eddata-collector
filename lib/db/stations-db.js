...
function migrateSchema () {
  ...
  // Check if engineer column exists
  const tableInfo = db.prepare('PRAGMA table_info(stations)').all()
  const hasEngineerColumn = tableInfo.some(col => col.name === 'engineer')

  if (!hasEngineerColumn) {
    console.log('Adding engineer column to stations table...')
    db.exec('ALTER TABLE stations ADD COLUMN engineer INT DEFAULT 0')
