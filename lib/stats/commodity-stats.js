...
async function updateCommodityStats () {
  ...
  // Create temporary table with non-Fleet Carrier market IDs for efficient filtering
  console.log('Creating temporary filter table...')
  tradeDb.exec(`
    DROP TABLE IF EXISTS temp_valid_markets;
    CREATE TEMP TABLE temp_valid_markets AS
    SELECT marketId FROM stationsDb.stations WHERE stationType != 'FleetCarrier';
    CREATE INDEX temp_valid_markets_idx ON temp_valid_markets(marketId);
  `)
  const count = tradeDb.prepare('SELECT COUNT(*) as count FROM temp_valid_markets').get()
  console.log(`Temporary table created with ${count.count} non-carrier stations`)

  // Updated queries to use temp table JOIN instead of IN clause for better performance
  const statements = {
    maxBuyPrice: tradeDb.prepare(`
      SELECT MAX(c.buyPrice) as maxBuyPrice
        FROM commodities c
        JOIN temp_valid_markets t ON c.marketId = t.marketId
