...
async function updateCommodityStats () {
  ...
  // Get list of non-Fleet Carrier market IDs once to avoid slow cross-database JOINs
  console.log('Fetching non-Fleet Carrier market IDs...')
  const nonCarrierMarketIds = tradeDb.prepare(`
    SELECT marketId FROM stationsDb.stations WHERE stationType != 'FleetCarrier'
  `).all().map(row => row.marketId)
  console.log(`Found ${nonCarrierMarketIds.length} non-carrier stations`)

  const marketIdList = nonCarrierMarketIds.join(',')

  // Updated queries to use IN clause instead of JOIN for better performance
  ...
  const statements = {
    maxBuyPrice: tradeDb.prepare(`
      ...
      WHERE c.commodityName = @commodityName COLLATE NOCASE
        AND c.marketId IN (${marketIdList})
        AND c.stock >= 1
        AND c.buyPrice > 0
