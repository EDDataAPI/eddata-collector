...
async function updateCommodityReport (
  ...
  if (!system.systemX || !system.systemY || !system.systemZ) {
    ...
    if (edsmData) {
      // Update database with coordinates only if not read-only (skip for snapshots)
      const isReadOnly = systemsDb.readonly || false
      ...
      if (!isReadOnly) {
        try {
          ...
          // Silently ignore write errors on read-only databases
        }
      }
      
      // Use EDSM data for calculations
      system.systemX = edsmData.systemX
      system.systemY = edsmData.systemY
      system.systemZ = edsmData.systemZ
    } else {
      // Silently skip if coordinates cannot be fetched
      return
    }
  }
