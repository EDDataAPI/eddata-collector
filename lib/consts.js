...
const TRADE_DATA_MAX_AGE_DAYS = 30
...
// Performance: Skip expensive index creation on startup
// Useful for faster first-time startup (indexes can be created later)
const SKIP_EXPENSIVE_INDEXES = process.env?.SKIP_EXPENSIVE_INDEXES === 'true'

// Performance: Skip database integrity checks on startup
// Only needed after crashes or corruption, safe to skip in production
const SKIP_INTEGRITY_CHECKS = process.env?.SKIP_INTEGRITY_CHECKS === 'true'

// Performance: Skip trade.db snapshots to prevent OOM on servers with limited RAM
// trade.db can be 7GB+ and snapshotting requires significant RAM
// Set to 'true' on servers with <16GB RAM to prevent OOM kills
const SKIP_TRADE_DB_SNAPSHOTS = process.env?.SKIP_TRADE_DB_SNAPSHOTS === 'true'
...
module.exports = {
  ...
  SKIP_EXPENSIVE_INDEXES,
  SKIP_INTEGRITY_CHECKS  SKIP_TRADE_DB_SNAPSHOTS,}
